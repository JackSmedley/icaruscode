 ...
 */
    ...
    for ( int i = 0; i < nFlashes; i++ ) {
      ...
      firstTimes.push_back( std::min(minTime, flashTime_Beam) ); //TODO: Understand why OpFlash.Time() is sometimes >2ns earlier than earliest hit

      if ( fVerbose ) std::cout << std::endl << "Flash time wrt trigger: " << flashTime_Trigger << ", Flash time wrt beam gate: " << flashTime_Beam << std::endl;
 ...
 */
    ...
    for ( int j = 0; j < nSlices; j++ ) {
      ...
      fChargeWidthX = pow( (sumPosSqr[0]/sumCharge - pow(sumPos[0]/sumCharge, 2)), .5);
      fChargeWidthY = pow( (sumPosSqr[1]/sumCharge - pow(sumPos[1]/sumCharge, 2)), .5);
      fChargeWidthZ = pow( (sumPosSqr[2]/sumCharge - pow(sumPos[2]/sumCharge, 2)), .5); 
      ...
      if ( isData ) matchedTime += shiftFactor;
